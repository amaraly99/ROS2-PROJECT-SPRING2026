...
# The per-run aggregate is the artifact's own summary of the sweep: a reader is
# handed this file, not the 65 per-run CSVs. It drifted once already (61 rows
# against 65 runs, with the paper's headline component values absent from it),
# so diff it here rather than trusting it to be regenerated by hand.
check "coco_v2_all_runs.csv" \
      "$TMP/acc/benchmarks/paper_data/accuracy/coco_v2_all_runs.csv" \
      "benchmarks/paper_data/accuracy/coco_v2_all_runs.csv"
