...
def find_sequences(cfg: dict) -> list:
    ...
    Supports three bag layouts (tried in order):
      1. Flat .db3  : <sequences_dir>/<name>.db3
      2. ROS2 dir   : <sequences_dir>/<name>/  (contains metadata.yaml)
      3. Nested .db3: <sequences_dir>/<name>/<name>.db3  ← EuRoC default
    Ground-truth is resolved as <gt_dir>/<name>.txt, falling back to
    <gt_dir>/<name>/<name>.txt for the nested layout.
    """
    ...
    if seqs_cfg == "all":
        ...
        if seq_dir.exists():
            for p in sorted(seq_dir.iterdir()):
                if p.suffix == ".db3":
                    # Flat .db3 directly in sequences_dir
                    names.append(p.stem)
                elif p.is_dir() and (p / "metadata.yaml").exists():
                    # ROS2 bag directory
                    names.append(p.name)
                elif p.is_dir() and (p / f"{p.name}.db3").exists():
                    # Nested layout: <seq_dir>/<name>/<name>.db3
                    names.append(p.name)
        if not names:
    ...
    result = []
    for name in names:
        # Resolve bag path — three layouts tried in priority order:
        #   1. Flat .db3        : <seq_dir>/<name>.db3
        #   2. ROS2 bag dir     : <seq_dir>/<name>/  with metadata.yaml present
        #   3. Nested .db3      : <seq_dir>/<name>/<name>.db3   ← EuRoC default
        bag = seq_dir / f"{name}.db3"                          # 1. flat
        if not bag.exists():
            _d = seq_dir / name
            if _d.is_dir() and (_d / "metadata.yaml").exists():
                bag = _d                                       # 2. valid ROS2 bag dir
            else:
                bag = _d / f"{name}.db3"                      # 3. nested
        # Resolve GT: flat → nested (<gt_dir>/<name>/<name>.txt)
        gt = gt_dir / f"{name}.txt"
        if not gt.exists():
            gt = gt_dir / name / f"{name}.txt"
        if not bag.exists():
...
def run_sequence(seq_name, bag_path, gt_path, cfg, ws, stamp,
    ...
    section(f"Pre-launch  [{seq_name}  run {run_num}/{n_retries}]")
    _w(f"  {C.DIM}Stereo preprocessing is managed by euroc_offline_f2m.launch.py{C.RESET}\n")
    ...
    timing_bag_proc = _bg(
        f"ros2 bag record --topics /odom_info /rtabmap/info -o {q(timing_bag_dir)}",
        logf("timing_bag"),
    )
