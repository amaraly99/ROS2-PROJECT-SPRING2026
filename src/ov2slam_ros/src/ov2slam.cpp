...
void SlamManager::run()
{
    ...
    while( !bexit_required_ ) {
        ...
        if( getNewImage(img_left, img_right, time) )
        {
            ...
            if( frame_id_ > 0 ) {
                cam_delay = nh->now().seconds() - last_img_time;
                last_img_time += cam_delay;
            } else {
                last_img_time = nh->now().seconds();
            }
            ...
            if( is_kf_req ) 
            {
                ...
                if( !bkf_viz_ison_ ) {
                    ...
                    pthread_setname_np(kf_viz_thread.native_handle(), "ov2_vizkf");
            ...
            if( !bframe_viz_ison_ ) {
                ...
                pthread_setname_np(viz_thread.native_handle(), "ov2_vizfr");
                viz_thread.detach();
            }
        } 
        else {
...
bool SlamManager::getNewImage(cv::Mat &iml, cv::Mat &imr, double &time)
{
    ...
    do {
        ...
        //time = nh->get_clock()->now().nanoseconds();
...
void SlamManager::visualizeAtKFsRate(const double time)
{
    ...
    // 👆👆👆 THIS FORCES THE 7CM DATA OUT LIVE 👆👆👆
...
{
    // 1. If nobody is listening, say nothing.
    if( prosviz_->pub_final_kfs_traj_->get_subscription_count() == 0 ) {
        return;
    }

    // 2. Get ONLY the latest Keyframe ID (The one just created)
    int last_kf_id = pcurframe_->kfid_;

    // 3. Grab the pointer to that specific keyframe
...
void SlamManager::writeResults()
{
    ...
    if( pslamstate_->do_full_ba_ ) 
    {
        ...
        prosviz_->pubPointCloud(pmap_->pcloud_, nh->now().seconds());
        visualizeFinalKFsTraj();
