...
Run the stereo SLAM node

    $ ros2 run orbslam stereo PATH_TO_VOCABULARY PATH_TO_STEREO_YAML_CONFIG_FILE

This node subscribes to the ROS2 topics `camera/left` and `camera/right` and expects
already-rectified image pairs. The included EuRoC stereo config lives at
`src/stereo/EuRoC.yaml`.

For EuRoC-style playback you will typically need to remap or republish your bag topics to
match the wrapper, for example from `/cam0/image_raw` and `/cam1/image_raw` to
`camera/left` and `camera/right` after rectification. Do not reuse the monocular
`src/monocular/EuRoC.yaml` file for stereo mode.
