...
public:
        ...
        : cameraposevisual_(1, 0, 0, 1)
    {
        ...
        // DEFINE A QOS PROFILE THAT ACTUALLY REMEMBERS SHT
        rclcpp::QoS qos_profile(rclcpp::KeepLast(1000)); 
        qos_profile.transient_local();  // <--- THIS IS THE MAGIC FIX 🪄
        qos_profile.reliable();

        // CREATE THE PUBLISHER WITH THE NEW PROFILE
        pub_final_kfs_pose_ = n->create_publisher<geometry_msgs::msg::PoseStamped>("final_kfs_pose", qos_profile);
    ...
    {   
        if (!pub_final_kfs_pose_) return;
        /*
        if( pub_final_kfs_traj_->get_subscription_count() == 0 ) {
            return;
        }
        // commented to allow for retrieving the points
        */
        ...
        Twc_msg.header.stamp = rclcpp::Time(time);
        ...
        pub_final_kfs_pose_->publish(Twc_msg);
    ...
    std::shared_ptr<rclcpp::Publisher<geometry_msgs::msg::PoseStamped>> pub_final_kfs_pose_;
